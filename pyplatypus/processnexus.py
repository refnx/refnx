...
class ProcessNexus(object):
	...
	def process(self, **kwds):
		"""
			...
			(self.M_lambda, self.M_lambdaSD, self.M_spec, self.M_specSD).

			Where lambda is the wavelength in Angstrom and M_spec is the intensity of that wavelength.
			SD is standard deviation.
			These each have the shape (N, T), where N is the number of scanpoints within the nexus file
			(unless the eventstreaming keyword is set) and	T is the number of wavelength/timebins in the rebinned data.
			
			The following object attributes are also set following successful processing with this method
				...
				self.M_specSD[N, T]			-corresponding SD for M_spec
				
				self.M_lambda[N, T]			-wavelength values for each bin
				
				self.M_lambdaSD[N, T]		-uncertainty in M_lambda
		...
		if self.normfilenumber:
			normfilename = 'PLP{0:07d}.nx.hdf'.format(int(abs(self.normfilenumber)))

			if self.basedir:
				for root, dirs, files in os.walk(self.basedir):
					if normfilename in files:
						self.normfilename = os.path.join(root, normfilename)
						break
		
		with h5.File(self.datafilename, 'r') as h5data:
			h5data = h5data

			self.frequency = h5data['entry1/instrument/disk_chopper/ch1speed'][0]
			
			scanpoint = 0
			
			#beam monitor counts for normalising data
			self.bmon1_counts = np.zeros(dtype = 'float64', shape = h5data['entry1/monitor/bm1_counts'].shape)
			self.bmon1_counts = h5data['entry1/monitor/bm1_counts'][:]
			...
			if 'eventstreaming' in kwds:
				...
				#detector(n, t, y, x)    
				self.detector = np.zeros(dtype='int32', shape = h5data['entry1/data/hmm'].shape)
				self.detector = h5data['entry1/data/hmm'][:]
			...
			for index in xrange(self.numspectra):
				...
				chod[index] = self.__chodcalculator(h5data, omega, two_theta, pairing, scanpoint)
	...
	#			assert not np.isnan(planeSD).any()
			...
			self.lopx = lopx
			self.hipx = hipx
			self.title = h5data['entry1/experiment/title'][0]
			self.sample = h5data['entry1/sample/name'][0]
			self.user = h5data['entry1/user/name'][0]
...
#		filename = 'PLP{:07d}_{:d}.spectrum'.format(self.datafilenumber, index)
	...
	def process_event_stream(self, tbins = None, xbins = None, ybins = None, frame_bins = None, scanpoint = 0):
		...
		with h5py.File(self.datafilename, 'r') as h5data:			
			if not tbins:
			...
			self.frequency = h5data['entry1/instrument/disk_chopper/ch1speed'][0] / 60
	...
	def __chodcalculator(self, h5data, omega, two_theta, pairing = 10, scanpoint = 0):
		chod = 0
		chopper1_distance = h5data['entry1/instrument/parameters/chopper1_distance']
		chopper2_distance = h5data['entry1/instrument/parameters/chopper2_distance']
		chopper3_distance = h5data['entry1/instrument/parameters/chopper3_distance']
